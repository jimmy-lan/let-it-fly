...
const loadUserTable = () => {
    return axios.get("/api/profiles").then(
        res => {
            const { body } = res.data;
            return {
                "data":
                    body.map(person => {
                        return {
                            ...
                            "contactinformation": person.contact.telephone + "/n " + person.contact.socialMedia,
                            "id": person.id,
                            "nickname": person.personal.name.last+' '+person.personal.name.first,
